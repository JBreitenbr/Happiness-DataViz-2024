...
let yAxis2=d3.axisLeft(yScale2);                  canvas.append('g').call(yAxis1).attr('transform','translate('+pad+',0)').style("font",`${(7/550)*h}`+"px montserrat");
                    canvas.append('g').call(yAxis2).attr('transform','translate('+0.625*w+',0)').style("font",`${(7/550)*h}`+"px montserrat");
...
canvas.append('text').attr('x',xScale1(0)).attr('y',yScale1(cnts1[0])-8*yScale1.bandwidth()/2).text(moodDict["happy"]).style("font",`${16*h/550}px arial`).style("fill","#333");
for(let i=lst1; i<lend1;i++){
canvas.append('text').attr('x',xScale1(rankDict[year][i][2])+5).attr('y',yScale1(rankDict[year][i][0])+yScale1.bandwidth()/2+3).text(rankDict[year][i][2]).style("font","8px arial").style("fill","#333");
canvas.append('text').attr('x',xScale1(0)+15).attr('y',yScale1(rankDict[year][i][0])+yScale1.bandwidth()/2+3).text(i+1).style("font","8px arial").style("text-anchor","end").style("fill","#333");
 }
...
canvas.append('text').attr('x',xScale2(rankDict[year][i][2])+5).attr('y',yScale2(rankDict[year][i][0])+yScale2.bandwidth()/2+3).text(rankDict[year][i][2]).style("font","8px arial").style("fill","#333");
canvas.append('text').attr('x',xScale2(0)+15).attr('y',yScale2(rankDict[year][i][0])+yScale2.bandwidth()/2+3).text(i+1).style("font","8px arial").style("text-anchor","end").style("fill","#333");
                    }
}  
showRank(year);
