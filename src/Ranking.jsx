...
let cnts=rankDict[year].slice().reverse().slice(0,30).map(d=>d[0]);
...
canvas.selectAll('rect').data(rankDict[year].slice().reverse().slice(0,30)).enter().append('rect').attr('x',xScale(0)).attr('y',d=>yScale(d[0])).attr('width',d=>xScale(d[2])).attr('height',yScale.bandwidth()).style("fill",d=>regDict[d[1]]).style("opacity",1);
