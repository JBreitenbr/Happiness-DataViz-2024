...
let cnts=rankDict[year].slice().reverse().slice(0,med+1).map(d=>d[0]);
...
canvas.selectAll('rect').data(rankDict[year].slice().reverse().slice(0,med+1)).enter().append('rect').attr('x',xScale(0)).attr('y',d=>yScale(d[0])).attr('width',d=>xScale(d[2])).attr('height',yScale.bandwidth()).style("fill",d=>regDict[d[1]]).style("opacity",1);

for(let i=l-1; i>=med;i--){
